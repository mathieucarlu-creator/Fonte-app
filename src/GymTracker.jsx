...
export default function GymTracker() {
  ...
  async function loadAll() {
    ...
    if (hasStorage) {
      ...
      for (const p of ["moi", "ben"]) {
        try {
          const res = await storage.get(`plan-${p}`);
          if (res && res.value) plans[p] = JSON.parse(res.value);
        } catch (e) {
          /* pas de séance planifiée enregistrée */
        }
      }
    } else {
  ...
  function persistPlan(who, plan) {
    if (!hasStorage) return;
    storage.set(`plan-${who}`, JSON.stringify(plan)).catch(() => {});
  }
  ...
  function addToDraft() {
    const m = draftMachine;
    if (!m || draftPlan.some((item) => item.machine === m)) return;
    const sug = computeSuggestion(entries[person], m);
    let w, r;
    if (sug.type !== "none") {
      w = sug.weight;
      r = sug.reps;
    } else {
      const hist = entries[person]
        .filter((e) => e.machine === m)
        .sort((a, b) => b.ts.localeCompare(a.ts));
      w = hist.length > 0 ? hist[0].weight : 20;
      r = hist.length > 0 ? hist[0].reps : 10;
    }
    setDraftPlan((prev) => [...prev, { machine: m, weight: w, reps: r, targetSets: 3 }]);
  }

  function adjustDraftSets(machine, delta) {
    setDraftPlan((prev) =>
      prev.map((item) => (item.machine === machine ? { ...item, targetSets: Math.max(1, item.targetSets + delta) } : item))
    );
  }

  function removeDraftItem(machine) {
    setDraftPlan((prev) => prev.filter((item) => item.machine !== machine));
  }

  function saveDraftPlan() {
    if (!draftPlan || draftPlan.length === 0) return;
    setSessionPlan((prev) => ({ ...prev, [person]: draftPlan }));
    persistPlan(person, draftPlan);
    if (draftPlan.length > 0) chooseMachine(draftPlan[0].machine);
    setDraftPlan(null);
  }
  ...
  return (
    <div className="gt-app" style={{ "--accent": accent, "--accent-soft": PEOPLE[person].accentSoft }}>
      ...
      {!activePlan && draftPlan === null && (
        <div className="gt-plan-starters">
          {mostRecentDate && (
          ...
          <button className="gt-quickstart gt-quickstart-outline" onClick={openPlanBuilder}>
            <PlusCircle size={16} />
      ...
      {draftPlan !== null && (
        <section className="gt-card gt-plan-card">
          <div className="gt-plan-header">
            ...
            <button className="gt-plan-quit" onClick={cancelDraftPlan}>
              Annuler
            </button>
          </div>

          <div className="gt-machine-row">
            <select className="gt-select" value={draftMachine} onChange={(e) => setDraftMachine(e.target.value)}>
            ...
            <button className="gt-icon-btn" onClick={addToDraft} aria-label="Ajouter à la séance">
              <PlusCircle size={20} />
            </button>
          </div>

          {draftPlan.length === 0 ? (
            <div className="gt-empty" style={{ marginTop: 10 }}>
              Ajoute des machines pour construire ta séance — le poids et les reps sont proposés d'après ta progression.
            </div>
          ) : (
            <div className="gt-plan-list" style={{ marginTop: 12 }}>
              {draftPlan.map((item) => (
                <div key={item.machine} className="gt-draft-item">
                  <div className="gt-draft-item-main">
                    <span className="gt-draft-item-name">{item.machine}</span>
                    <span className="gt-draft-item-ref">
                      {fmtWeight(item.weight)}kg × {item.reps}
                    </span>
                  </div>
                  <div className="gt-draft-item-sets">
                    <button className="gt-step-btn" onClick={() => adjustDraftSets(item.machine, -1)} aria-label="Moins une série">
                      <Minus size={14} />
                    </button>
                    <span className="gt-draft-sets-count">{item.targetSets} séries</span>
                    <button className="gt-step-btn" onClick={() => adjustDraftSets(item.machine, 1)} aria-label="Plus une série">
                      <Plus size={14} />
                    </button>
                    <button className="gt-trash" onClick={() => removeDraftItem(item.machine)} aria-label="Retirer la machine">
                      <Trash2 size={15} />
                    </button>
                  </div>
                </div>
          ...
          {draftPlan.length > 0 && (
            <button className="gt-btn gt-btn-accent gt-btn-full" style={{ marginTop: 14 }} onClick={saveDraftPlan}>
              Enregistrer la séance ({draftPlan.length} machine{draftPlan.length > 1 ? "s" : ""})
            </button>
          )}
        </section>
      )}
      ...
      {activePlan && (
        <section className="gt-card gt-plan-card">
          ...
          <div className="gt-plan-list">
            {activePlan.map((item) => {
              const target = item.targetSets || 1;
              const doneCount = personEntries.filter((e) => e.date === today && e.machine === item.machine).length;
              const complete = doneCount >= target;
              const isCurrent = selectedMachine[person] === item.machine;
              return (
                <button
                  key={item.machine}
                  className={`gt-plan-item${complete ? " gt-plan-item-done" : ""}${isCurrent ? " gt-plan-item-current" : ""}`}
                  onClick={() => chooseMachine(item.machine)}
                >
                  {complete ? <Check size={14} /> : <span className="gt-plan-dot" />}
                  <span className="gt-plan-item-name">{item.machine}</span>
                  <span className="gt-plan-item-ref">
                    {fmtWeight(item.weight)}kg × {item.reps} · {doneCount}/{target}
                  </span>
                </button>
              );
            })}
          </div>
          {activePlan.every((item) => {
            const target = item.targetSets || 1;
            const doneCount = personEntries.filter((e) => e.date === today && e.machine === item.machine).length;
            return doneCount >= target;
          }) && <div className="gt-plan-complete">Séance terminée 🎉 Bien joué !</div>}
        </section>
      )}
...
.gt-plan-starters { display: flex; flex-direction: column; gap: 8px; margin-bottom: 14px; }
.gt-quickstart {
  display: flex;
  align-items: center;
  gap: 9px;
...
.gt-quickstart-outline { background: transparent; color: var(--text-muted); border-style: dashed; }
...
.gt-draft-item-main { display: flex; flex-direction: column; gap: 2px; min-width: 0; }
.gt-draft-item-name { font-size: 13.5px; font-weight: 500; color: var(--text); }
.gt-draft-item-ref {
  font-family: 'JetBrains Mono', monospace;
  font-size: 11.5px;
  color: var(--text-muted);
}
.gt-draft-item-sets { display: flex; align-items: center; gap: 6px; flex-shrink: 0; }
.gt-draft-sets-count {
  font-family: 'JetBrains Mono', monospace;
  font-size: 11.5px;
  color: var(--text);
