...
import { Search, Plus, Trash2, ChevronLeft, ChevronRight, Loader2, Check, X, BookOpen, Camera } from "lucide-react";
...
export default function FoodJournal({ calorieTarget, macroTargets }) {
  ...
  async function runSearch(explicitQuery) {
    ...
    try {
      ...
      } else if (results.length === 1 && /^\d{8,14}$/.test(q)) {
        // Code-barres : un seul produit possible, on saute directement à la quantité
        pickFood(results[0], true);
      }
    } catch (e) {
  ...
  async function startScanner() {
    ...
    try {
      ...
      const backCamera = devices.find((d) => /back|rear|environment|arrière/i.test(d.label)) || devices[devices.length - 1];

      readerRef.current.decodeFromVideoDevice(backCamera.deviceId, videoRef.current, (result) => {
        // Le scanner continue d'analyser l'image plusieurs fois par seconde et
        // peut redéclencher ce callback pour la même détection avant que la
        // caméra n'ait fini de s'arrêter — ce verrou garantit une seule
        // recherche déclenchée par scan, même si le callback est encore
        // appelé une ou deux fois de plus juste après.
        ...
        // Les erreurs "pas de code sur cette image" sont normales à chaque frame, on les ignore.
      });
    } catch (e) {
      setScanError("Impossible d'accéder à la caméra. Vérifie que tu as autorisé l'accès, ou utilise la recherche texte ci-dessous.");
      setScanning(false);
    }
  }
    ...
    <div className="fj-wrap">
      ...
      <section className="fj-card">
        ...
        <div className="fj-search-row">
          ...
          <button className="fj-icon-btn" onClick={startScanner} disabled={searching} aria-label="Scanner un code-barres">
            <Camera size={18} />
          </button>
        </div>

        {scanError && (
          <div className="fj-notice" style={{ marginTop: 10 }}>
            {scanError}
            <button className="fj-notice-close" onClick={() => setScanError("")} aria-label="Fermer">
              ×
            </button>
          </div>
        )}
        ...
        {scanning && (
          <div className="fj-scanner">
            <video ref={videoRef} className="fj-scanner-video" playsInline muted />
            <div className="fj-scanner-frame" />
            <button className="fj-btn fj-btn-accent fj-btn-full" style={{ marginTop: 10 }} onClick={stopScanner}>
              Annuler le scan
            </button>
          </div>
        )}
...
.fj-scanner { margin-top: 10px; position: relative; border-radius: 12px; overflow: hidden; background: #000; }
.fj-scanner-video { width: 100%; display: block; max-height: 320px; object-fit: cover; }
.fj-scanner-frame {
