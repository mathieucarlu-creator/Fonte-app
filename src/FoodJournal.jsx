...
async function searchOpenFoodFacts(query) {
  ...
  if (isBarcode) {
    ...
    const result = productToResult(data.product);
    return result ? [result] : [];
  }

  const res = await fetch(
    `https://world.openfoodfacts.org/cgi/search.pl?search_terms=${encodeURIComponent(trimmed)}&search_simple=1&action=process&json=1&page_size=15`
  );
  if (!res.ok) throw new Error("Recherche indisponible (réseau)");
  const data = await res.json();
