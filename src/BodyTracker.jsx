...
function fmtNum(n) {
  ...
  return Number.isInteger(rounded) ? String(rounded) : String(rounded).toFixed(1).replace(".", ",");
}
            ...
            <div key={group.title} className="bt-form-group">
              ...
              <div className="bt-form-grid">
                {group.fields.map((key) => {
                  ...
                  return (
                    <label key={key} className="bt-field">
...
.bt-quickstart {
  ...
  border-radius: 12px; padding: 13px 14px; font-weight: 600; font-size: 13.5px; margin-bottom: 16px;
}
